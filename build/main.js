...
class Pondpump extends utils.Adapter {
  ...
  async onReady() {
    ...
    this.subscribeStates("pumps.*.schedule.manual");
    this.log.info(
  ...
  async runScheduler() {
    ...
    for (const [dnStr, cfg] of Object.entries(this.schedules)) {
      ...
      const manual = ((_h = await this.getStateAsync(`pumps.${deviceNumber}.schedule.manual`)) == null ? void 0 : _h.val) === true;
      if (manual) {
        this.log.debug(`[schedule] pump ${deviceNumber}: manual override \u2014 scheduler not applying power/SFC`);
      } else {
        let resolvedPower;
        const rampPph = (_i = cfg.rampPercentPerHour) != null ? _i : 0;
  ...
  async applyScheduleTarget(deviceNumber, sfc, power, rt, nowMs) {
    ...
    const actualSfc = (sfcState == null ? void 0 : sfcState.val) === true;
    const speedState = await this.getStateAsync(`${base}.speed`);
    const actualSpeed = typeof (speedState == null ? void 0 : speedState.val) === "number" ? speedState.val : void 0;
    rt.appliedSfc = sfc;
    if (!sfc && power !== void 0) {
    ...
    if (!sfc && power !== void 0 && actualSpeed !== power) {
      await this.setState(`${base}.speed`, { val: power, ack: false });
      changes.push(`power ${actualSpeed != null ? actualSpeed : "?"} % \u2192 ${power} %`);
    }
    if (changes.length) {
   ...
   */
  async onManualFlagCommand(deviceNumber, manual) {
   ...
   */
  noteManualControlOverride(id, val) {
    if (!this.scheduleStarted || this.stopping) {
      return;
    }
    ...
    if (!(cfg == null ? void 0 : cfg.enabled) || !(0, import_schedule.validatePlans)(cfg.plans || []).valid || !this.pumpControl.has(deviceNumber)) {
      return;
    }
